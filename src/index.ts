...
const checkUserAgentMobile = (agent: string) => {
...
const checkResulutionMobile = () => {
    return window.innerWidth <= 800 && window.innerHeight <= 600;
}

const checkMobile = () => {
    const userAgentMobile = checkUserAgentMobile(navigator.userAgent || navigator.vendor);
    const resulutionMobile = checkResulutionMobile();
    return userAgentMobile || resulutionMobile;
}

const start = () => {
    if(checkMobile() && window.location.href.match(/[^\/]*$/)![0] !== 'mobile') {
